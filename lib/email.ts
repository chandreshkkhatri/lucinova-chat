...
export async function sendBadgeEarnedEmail(
  ...
  // Check if Resend is configured
  if (!resend) {
    console.warn("RESEND_API_KEY not configured. Skipping badge earned email send.");
    return { success: false, error: "Email service not configured" };
  }
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || "Lucidity <onboarding@resend.dev>",
      to: [to],
      subject: `🎉 You've Earned the ${badgeDetails.badgeName} Badge!`,
      html: getBadgeEarnedEmailTemplate(displayName, badgeDetails),
    });
...
function getBadgeEarnedEmailTemplate(
  displayName: string,
  badgeDetails: {
    badgeName: string;
    badgeIcon: string;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding: 40px 40px 20px 40px; text-align: center;">
              ...
              <h1 style="margin: 0; color: #1a1a1a; font-size: 28px; font-weight: 700;">
                You've Earned the ${badgeDetails.badgeName} Badge!
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 0 40px 30px 40px; color: #718096; font-size: 16px; text-align: center; line-height: 1.6;">
              <p style="margin: 0;">
                Congratulations on becoming part of an exclusive group!
              </p>
            </td>
          </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                <tr>
                  <td style="padding: 20px; color: #4a5568; font-size: 15px;">
                    ...
                    <p style="margin: 0 0 8px 0;">
                      <strong>Badge:</strong> ${badgeDetails.badgeName}
                    </p>
                    ${badgeDetails.userRank ? `<p style="margin: 0 0 8px 0;"><strong>Your Rank:</strong> User #${badgeDetails.userRank} of 500</p>` : ""}
                    ${badgeDetails.benefit ? `<p style="margin: 0 0 8px 0;"><strong>Benefit:</strong> ${badgeDetails.benefit}</p>` : ""}
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 0 40px 30px 40px; color: #718096; font-size: 14px; line-height: 1.6;">
              ...
              <p style="margin: 0;">
                Keep using Lucidity to unlock more badges and exclusive benefits. Check your account settings to see all your earned badges!
              </p>
            </td>
          </tr>
